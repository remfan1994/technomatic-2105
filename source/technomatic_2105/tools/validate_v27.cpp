...
int main() {
    ...
    std::set<uint64_t> tensionIds, chordRhythmIds;
    std::set<uint64_t> timbreIds, padVoicingIds;
    std::set<int32_t> bassModels, leadModels, padModels, drumKits;
    int32_t padBreathDeviceCount = 0;
    int32_t sustainedChordArticulationCount = 0;
    int32_t oldStabLatticeCount = 0;
    ...
    for (uint32_t seed = 1; seed <= kSeeds; ++seed) {
        ...
        uint64_t timbreHash = 1469598103934665603ull;
        timbreHash = mix64(timbreHash, diversity.mComposition.timbreGrammarSeed);
        timbreHash = mix64(timbreHash, static_cast<uint32_t>(diversity.mComposition.bassModel));
        timbreHash = mix64(timbreHash, static_cast<uint32_t>(diversity.mComposition.leadModel));
        timbreHash = mix64(timbreHash, static_cast<uint32_t>(diversity.mComposition.padModel));
        timbreHash = mix64(timbreHash, static_cast<uint32_t>(diversity.mComposition.drumKit));
        timbreHash = mix64(timbreHash, static_cast<uint32_t>(std::lround(diversity.mComposition.bassMotion * 4096.0f)));
        timbreHash = mix64(timbreHash, static_cast<uint32_t>(std::lround(diversity.mComposition.leadModRatio * 4096.0f)));
        timbreHash = mix64(timbreHash, static_cast<uint32_t>(std::lround(diversity.mComposition.padMotion * 4096.0f)));
        timbreIds.insert(timbreHash);
        padVoicingIds.insert(hashInts(diversity.mComposition.padIntervals));
        bassModels.insert(diversity.mComposition.bassModel);
        leadModels.insert(diversity.mComposition.leadModel);
        padModels.insert(diversity.mComposition.padModel);
        drumKits.insert(diversity.mComposition.drumKit);

        uint64_t tensionHash = hashPhrase(diversity.mComposition.tensionRel,
                                          diversity.mComposition.tensionGate);
        tensionHash = mix64(tensionHash, static_cast<uint32_t>(diversity.mComposition.tensionPrimary));
        tensionHash = mix64(tensionHash, static_cast<uint32_t>(diversity.mComposition.tensionSecondary));
        tensionHash = mix64(tensionHash, static_cast<uint32_t>(diversity.mComposition.tensionCycle));
        tensionHash = mix64(tensionHash, static_cast<uint32_t>(diversity.mComposition.tensionStride));
        tensionIds.insert(tensionHash);

        uint64_t chordHash = 1469598103934665603ull;
        for (float gate : diversity.mComposition.chordGate) {
            chordHash = mix64(chordHash, static_cast<uint32_t>(std::lround(gate * 4096.0f)));
        }
        chordHash = mix64(chordHash, static_cast<uint32_t>(diversity.mComposition.chordArticulation));
        chordRhythmIds.insert(chordHash);

        if (diversity.mComposition.tensionPrimary == static_cast<int32_t>(rb::MusicEngine::TensionDevice::PadBreath) ||
            diversity.mComposition.tensionSecondary == static_cast<int32_t>(rb::MusicEngine::TensionDevice::PadBreath)) {
            ++padBreathDeviceCount;
        }
        if (diversity.mComposition.chordArticulation == 3) ++sustainedChordArticulationCount;

        bool oldLattice = true;
        for (int32_t pos = 0; pos < 16; ++pos) {
            const bool oldHit = pos == 2 || pos == 6 || pos == 10 || pos == 14;
            if (oldHit != (diversity.mComposition.tensionGate[pos] > 0.02f)) {
                oldLattice = false;
                break;
            }
        }
        if (oldLattice) ++oldStabLatticeCount;
    }
    ...
    require(tensionIds.size() >= kSeeds * 990 / 1000,
            "generated tension grammars are effectively unique");
    require(chordRhythmIds.size() >= kSeeds * 750 / 1000,
            "generated harmonic rhythms have broad diversity");
    require(timbreIds.size() >= kSeeds * 995 / 1000,
            "generated timbre grammars are effectively unique");
    require(bassModels.size() == 12, "bass synthesis bank has broad model coverage");
    require(leadModels.size() == 16, "lead synthesis bank has broad model coverage");
    require(padModels.size() == 10, "pad synthesis bank has broad model coverage");
    require(drumKits.size() == 8, "all generated drum-kit families are reachable");
    require(padVoicingIds.size() >= 32, "generated pad voicings have broad harmonic diversity");
    require(oldStabLatticeCount == 0,
            "the old 2/6/10/14 tension lattice is absent");
    require(padBreathDeviceCount < static_cast<int32_t>(kSeeds / 5),
            "sustained pad tension is rare");
    require(sustainedChordArticulationCount < static_cast<int32_t>(kSeeds / 4),
            "sustained pad chord articulation is uncommon");

    // Regression for the reported seed and timestamp. v25 produced the
    // recognizable overlapping pad pulse around 1:27. v27 preserves the same
    // selected candidate while scheduling no PadVoice in that interval.
    rb::MusicEngine reported;
    reported.prepare(48000.0);
    reported.reset(3290437499u);
    require(reported.mCurrentCandidateIndex == 44,
            "reported seed retains candidate 44 after the targeted rewrite");
    std::vector<float> reportedBlock(256 * 2, 0.0f);
    int64_t reportedFrames = 0;
    constexpr int64_t reportedBegin = 82LL * 48000LL;
    constexpr int64_t reportedEnd = 92LL * 48000LL;
    int32_t maxPads = 0;
    int32_t intervalPadOnsets = 0;
    int32_t previousPads = 0;
    while (reportedFrames < reportedEnd) {
        const int32_t frames = static_cast<int32_t>(std::min<int64_t>(256, reportedEnd - reportedFrames));
        std::fill(reportedBlock.begin(), reportedBlock.end(), 0.0f);
        reported.render(reportedBlock.data(), frames, 2);
        reportedFrames += frames;
        if (reportedFrames >= reportedBegin) {
            int32_t activePads = 0;
            for (const auto& pad : reported.mPads) if (pad.active) ++activePads;
            maxPads = std::max(maxPads, activePads);
            if (activePads > previousPads) ++intervalPadOnsets;
            previousPads = activePads;
        }
    }
    require(intervalPadOnsets == 0,
            "reported 1:22-1:32 interval contains no sustained pad onset");

    // Slow pads are a rare single breath, never a stacked repeating wall.
    rb::MusicEngine padStress;
    padStress.prepare(48000.0);
    int32_t globalMaxPads = maxPads;
    std::vector<float> padBlock(512 * 2, 0.0f);
    for (uint32_t seed = 1; seed <= 12; ++seed) {
        padStress.reset(seed * 2246822519u);
        for (int32_t blockIndex = 0; blockIndex < 8 * 48000 / 512; ++blockIndex) {
            std::fill(padBlock.begin(), padBlock.end(), 0.0f);
            padStress.render(padBlock.data(), 512, 2);
            int32_t activePads = 0;
            for (const auto& pad : padStress.mPads) if (pad.active) ++activePads;
            globalMaxPads = std::max(globalMaxPads, activePads);
        }
    }
    require(globalMaxPads <= 1, "slow pad voices remain concurrency-limited");
    ...
    constexpr int32_t kLongRenderSeconds = 180;
    ...
    require(indefinite.currentElapsedSeconds() > 170.0,
    ...
    const std::string pcmPath = "/tmp/technomatic_v27_export_test.pcm";
    ...
    std::cout << "INFO unique tension/chord rhythms: " << tensionIds.size() << "/" << chordRhythmIds.size() << " of " << kSeeds << "\n";
    std::cout << "INFO unique timbres/pad voicings: " << timbreIds.size() << "/" << padVoicingIds.size() << " of " << kSeeds << "\n";
    std::cout << "INFO model coverage bass/lead/pad/drum: " << bassModels.size() << "/" << leadModels.size() << "/" << padModels.size() << "/" << drumKits.size() << "\n";
    std::cout << "INFO PadBreath device/articulation counts: " << padBreathDeviceCount << "/" << sustainedChordArticulationCount << " of " << kSeeds << "\n";
    ...
    std::cout << (ok ? "PASS v27 validation\n" : "FAIL v27 validation\n");
    return ok ? 0 : 1;
}
